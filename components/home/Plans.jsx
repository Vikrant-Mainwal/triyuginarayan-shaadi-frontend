...
const Plans = () => {
  ...
  <div
    key={index}
    className="flex flex-col justify-between items-center rounded-[21px] bg-white shadow-[0px_4px_31.8px_rgba(0,0,0,0.18)] h-[970px] md:h-[1200px] w-[316px] md:w-[550px] mt-10 p-3"
  >
    
    <div className="flex flex-col items-center gap-4 mt-10">
      <h2 className="text-[26px] md:text-[45px]">{item.title}</h2>
      <p className="text-[14px] md:text-[18px] text-center p-[12px]">{item.description}</p>
      <h3 className="text-[48px] md:text-[67px] text-[#A42D2B]">{item.price}</h3>
      <div className="flex gap-5">
        <p className="flex text-[12px] md:text-[16px]">
        <Image src={Pepicons} alt="image"/><span>{item.nights} Night </span>
        </p>
        <p className="flex text-[12px] md:text-[16px]">
        <Image src={Calender} alt="image"/><span>{item.guests}</span>
        </p>

      </div>

      <div className="h-[2px] bg-[#EACAB9] w-[250px] md:w-[500px]" />
      <h5 className="text-[#A42D2B] py-3 md:py-6 text-[20px] font-medium">Package Includes</h5>
      <ul className="flex flex-col text-start px-5" >
        {item.features.map((feature, i) => {
            return(
            <div className="flex gap-4 md:p-2" key={i}>
            <Image src={Tick} alt="image" className="w-[27px] md:w-[30px]"/>
            <li className="text-[12px] md:text-[16px]">{feature}</li>
            </div>
